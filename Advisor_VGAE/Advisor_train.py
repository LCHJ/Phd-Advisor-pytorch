...
def Advisor_main(X_Inputs, T_Truths, A_ThesisFramework, Z_InitKnowledge):
    ...
    A_ThesisFramework = torch.where(A_ThesisFramework > k[-40], A_ThesisFramework / k[-3], A_ThesisFramework)
    ...
    print("===>>> acc_init= {} | nmi_init= {} <<<===".format([float('{:.5f}'.format(i * 100)) for i in config.acc_init], [float('{:.5f}'.format(i * 100)) for i in config.nmi_init]))
