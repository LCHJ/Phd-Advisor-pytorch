...
def Advisor_main(X_Inputs, T_Truths, A_ThesisFramework, Z_InitKnowledge):
    ...
    k, _ = torch.sort(A_ThesisFramework[10])
    A_ThesisFramework = torch.where(
        A_ThesisFramework > k[-100], A_ThesisFramework / k[-3], A_ThesisFramework
    )
    ...
    # origin ACC & NMI
    config.acc_init, config.nmi_init = cal_clustering_metric(
        1, T_Truths, A_ThesisFramework, X_Inputs
    )
    print(
        "===>>> acc_init= {} | nmi_init= {} <<<===".format(
            [float("{:.5f}".format(i * 100)) for i in config.acc_init],
    ...
    with open(config.save_path + "/Advisor_epoch_result.txt", "a+") as f:
        f.write(
            ...
            + "acc_init="
            + str([float("{: .5f}".format(i * 100)) for i in config.acc_init])
            + "\t nmi_init="
            + str([float("{: .5f}".format(i * 100)) for i in config.nmi_init])
            + "\n================================================\n"
        )
    ...
    pos_weight = (
        float(
            A_ThesisFramework.shape[0] * A_ThesisFramework.shape[0]
            - A_ThesisFramework.sum()
        )
        / A_ThesisFramework.sum()
    )

    norm = (
        A_ThesisFramework.shape[0]
        * A_ThesisFramework.shape[0]
        / float(
            (
                A_ThesisFramework.shape[0] * A_ThesisFramework.shape[0]
    ...
    model = GCNModelVAE(
        dim_features, config.hidden1, config.hidden2, config.Advisor_dropout
    )
    if Cuda:
    ...
    lr_scheduler = optim.lr_scheduler.StepLR(
        optimizer, step_size=config.Advisor_step_size, gamma=config.Advisor_gamma
    )
    ...
    for epoch in range(config.Advisor_epochs):
        ...
        if epoch % config.iteration == 0:
            ...
            with torch.no_grad():
                ...
                hidden_emb = mu.data
                try:
                    acc_curr, nmi_curr = cal_clustering_metric(
                        epoch, T_Truths, recovered, hidden_emb
                    )
                    ...
                    print(
                        ...
                        "%04d" % (epoch + 1),
                        "lr=",
                        "{:.5f}".format(get_lr(optimizer)),
                        "t_loss=",
                        "{:.5f}".format(cur_loss),
                        "acc_curr=",
                        [float("{:.5f}".format(i)) for i in acc_curr],
                        "\tnmi_curr=",
                        [float("{:.5f}".format(i)) for i in nmi_curr],
                        "time=",
                    ...
                    if config.great:
                        ...
                        with open(
                            config.save_path + "/Advisor_epoch_result.txt", "a+"
                        ) as f:
                            f.write(
                                "acc="
                                + str(
    ...
    print(
        ...
        "\nbest_acc",
        "\t\tbest_nmi\n",
        [float("{:.5f}".format(i * 100)) for i in config.best_acc],
        [float("{:.5f}".format(i * 100)) for i in config.best_nmi],
    )
