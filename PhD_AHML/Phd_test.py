...
def evaluate(net, loss, data, label, num_train):
    ...
    with tqdm(total=1, desc=f"1 {0 + 1}/{1}", postfix=dict, mininterval=0.3) as pbar:
        ...
        with torch.no_grad():
            x_reconstruction, Z_InitKnowledge, mu, logvar = net(
                X_Inputs
            )  # train and forward pass
            A = distance(Z_InitKnowledge)
            output, A_ThesisFramework = loss(x_reconstruction, X_Inputs, A, mu, logvar)

        acc, nmi = cal_clustering_metric(
            0, T_Truths, A_ThesisFramework, Z_InitKnowledge
        )
...
def Phd_test():
    ...
    X_Inputs, T_Truths, A_ThesisFramework, Z_InitKnowledge = evaluate(
        AH_net, loss, data, label, num_train
    )
