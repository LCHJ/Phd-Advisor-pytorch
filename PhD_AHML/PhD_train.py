...
def train_unit(net, loss, images, targets, epoch, optimizer):
    ...
    if not config.eval:
        ...
        with torch.no_grad():  # Intermediate empty data needs to be filled when generational clustering
            acc, nmi = cal_clustering_metric(epoch, targets, Y, mu.data)
            if config.great and epoch % 4 == 0:
                # show effect of refactoring
                index = ids[::10][:6]
                contrast_show(
                    x_reconstruction[index].permute(0, 2, 3, 1),
                    images[index].permute(0, 2, 3, 1),
                )
                plot_embeddings(
                    Z.cpu().detach().numpy().astype(float),
                    targets.cpu().detach().numpy().astype(int).flatten(),
                    "Z",
                    epoch,
                )
...
def fit_one_epoch_mat(
    net, loss, epoch, epoch_size, data, label, num_train, Epoch, optimizer
):
    ...
    with tqdm(
        ...
        for iteration in range(0, epoch_size):
            ...
            ids = np.array(
                random.sample(range(num_train), min(num_train, config.Batch_size))
            )
            ...
            targets = label[ids]
            # train
            o_loss, acc, nmi, Y, Adj = train_unit(
                net, loss, images, targets, epoch, optimizer
            )
            # result
            total_acc += acc
            total_nmi += nmi
            ...
            pbar.set_postfix(
                **{
                    ...
                    "A_mean": float("{:.5f}".format(Adj.mean())),
                    "Y_mean": float("{:.5f}".format(Y.mean())),
                    "loss": [float("{:.6f}".format(Train_loss / (iteration + 1)))],
                    "lr": get_lr(optimizer),
                }
            )
            pbar.update(1)
    ...
    if int(epoch) % 2 == 0:
        ...
        config.eval = True
        with tqdm(
            total=1, desc=f"Epoch {epoch + 1}/{Epoch}", postfix=dict, mininterval=0.3
        ) as pbar:
            ...
            with torch.no_grad():
                o_loss, acc, nmi, Y, Adj = train_unit(
                    net, loss, X_Inputs, T_Truths, epoch, optimizer
                )
            ...
            pbar.set_postfix(
                **{
                    "acc": [float("{:.5f}".format(i)) for i in (100 * total_acc)],
                    "nmi": [float("{:.5f}".format(i)) for i in (100 * total_nmi)],
                    "loss": [float("{:.6f}".format(Train_loss))],
                    "lr": get_lr(optimizer),
                }
            )
            pbar.update(1)
            ...
            if config.great:
                np.save(
                    config.save_path + "/" + "X_Inputs.npy",
                    X_Inputs.cpu().detach().numpy().astype(np.float32),
                )
                plt.close()
            ...
            tqdm.write(
                "\t>>-- y \t z_sp \t z_kmeans --<< "
                + "\nbest_acc ="
                + str([float("{:.5f}".format(i)) for i in 100 * config.best_acc])
                + "\nbest_nmi ="
                + str([float("{:.5f}".format(i)) for i in 100 * config.best_nmi])
            )
    else:
    ...
    end_time = time.strftime(
        "%H:%M:%S", time.gmtime(time_elapsed * (Epoch - epoch - 1))
    )
    tqdm.write(
        "---> Training complete in [{}] --- Estimated time in [{}] <---".format(
            str(spend_time), str(end_time)
        )
    )
...
def PhD_main():
    ...
    optimizer = optim.Adam(
        AH_net.parameters(), config.D_lr, weight_decay=config.D_weight_decay
    )
    # lr_scheduler = torch.optim.lr_scheduler.ExponentialLR(
    #     optimizer, gamma=config.D_gamma, last_epoch=-1
    # )
    lr_scheduler = optim.lr_scheduler.StepLR(
        optimizer, step_size=config.D_step_size, gamma=config.D_gamma
    )

    # print model construction
    summary(
        AH_net,
        (config.input_shape[2], config.input_shape[0], config.input_shape[1]),
    )
    ...
    os.makedirs(
        config.save_path
        + "/acc="
        + str([float("{: .5f}".format(i)) for i in config.best_acc])
        + "nmi="
        + str([float("{: .5f}".format(i)) for i in config.best_nmi])
    )
