...
class DefaultConfigs(object):
    ...
    def __init__(self, data_name=None, is_T=False, is_img_T=False, is_rgb=False, is_negative=False, ):
        # ========================  General Phd config =====================================
        # 1.data path
        ...
        if self.data_name == "umist_1024":
            ...
            self.iteration_rate = (20  # Number of iterations of training set and validation set
                                   )
        ...
        if self.data_name == "COIL20":
            ...
            self.Init_Epoch = 0
            self.Unfreeze_Epoch = 401
            self.iteration_rate = (10  # Number of iterations of training set and validation set
                                   )
            self.epoch_size_val = 1
        ...
        if self.data_name == "USPS":
            ...
            self.Init_Epoch = 0
            self.Unfreeze_Epoch = 501
            self.iteration_rate = (5  # Number of iterations of training set and validation set
                                   )
            self.epoch_size_val = 1
        ...
        if self.data_name == "fashion-mnist":
            ...
            self.Init_Epoch = 0
            self.Unfreeze_Epoch = 501
            self.iteration_rate = (4  # Number of iterations of training set and validation set
                                   )
            self.epoch_size_val = 1
            self.Batch_size = 1000
            # 6 hyper-parameter
        ...
        self.iteration = 10
        self.Advisor_epochs = self.iteration * 500
        self.Advisor_lr = 1e-4
        self.Advisor_step_size = self.iteration * 10
        self.Advisor_gamma = 0.99
        
        self.my_path = "./logs/{}".format(str(self.data_name))
        self.Phd_path = self.my_path + "/{}/Rho-{}_gama-{}_mse_line2_mu_1000/".format("Phd", str(self.margin),
                                                                                      str(self.Mul_MSN))
        self.Advisor_path = self.my_path + "/{}/h1-{}_h2-{}".format("Advisor", self.hidden1, self.hidden2)
        # path of saving model
        self.model_path = "./model/{}/".format(self.data_name)
        self.save_path = self.Phd_path


args = ["umist_1024", "COIL20", "Palm", "fashion-mnist", "USPS", "MSRA", "USPSdata_20_uni", "mnist_test", "CIFAR_test",
        "imm40", "JAFFE", ]

config = DefaultConfigs(args[0])
